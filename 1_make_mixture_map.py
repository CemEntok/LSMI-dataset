...
CAMERA = "/mnt/ssd-storage/cem/dataset/galaxy"
RAW = CAMERA+".dng"
VISUALIZE = False
...
def get_illumination_map(place, placeInfo):
    ...
    # make illumination map (1,12,13,123 pair)
        ...
        #Uce
        # breakpoint()
        # np.where(np.logical_and(coefficient_1>-1, coefficient_1<=-1e-16))
        # if len(np.where(coefficient_1<0)[0])>0:
        #     print("Uce")
        coefficient_1 = np.where(np.logical_and(coefficient_1>-1, coefficient_1<=-1e-17), 0, coefficient_1)
        if len(np.where(np.logical_and(coefficient_1>-1, coefficient_1<=-1e-17))[0])>0:
            print("minus coefficient!!!")
            breakpoint()
        #Uce
